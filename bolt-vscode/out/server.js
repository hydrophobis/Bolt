...
function parseDocument(doc) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (inString)
            inString = false; // reset per line (simple heuristic)
        ...
        for (const ch of stripped) {
            if (ch === '{')
                ...
                if (braceDepth < 0) {
                    diagnostics.push({
                        range: node_1.Range.create(i, stripped.indexOf('}'), i, stripped.indexOf('}') + 1),
    ...
    if (braceDepth > 0) {
        diagnostics.push({
            range: node_1.Range.create(lines.length - 1, 0, lines.length - 1, 0),
...
function getWordAtPosition(doc, pos) {
    const line = doc.getText(node_1.Range.create(pos.line, 0, pos.line, 9999));
...
connection.onCompletion((params) => {
    ...
    if (!doc)
        return [];
    const line = doc.getText(node_1.Range.create(params.position.line, 0, params.position.line, params.position.character));
    ...
    for (const t of PRIMITIVE_TYPES) {
        items.push({ label: t, kind: node_1.CompletionItemKind.TypeParameter });
    }
    // Symbols from current document
    const symbols = symbolCache.get(params.textDocument.uri) ?? [];
    for (const sym of symbols) {
        const kind = sym.kind === 'function' ? node_1.CompletionItemKind.Function :
            sym.kind === 'class' || sym.kind === 'struct' ? node_1.CompletionItemKind.Class :
    ...
    if (isMemberAccess) {
        ...
        for (const m of selfMembers) {
            items.push({ label: m.name, kind: node_1.CompletionItemKind.Field, detail: m.detail });
        }
    }
    return items;
});
...
connection.onHover((params) => {
    ...
    if (HOVER_DOCS[word]) {
        return {
            contents: { kind: node_1.MarkupKind.Markdown, value: HOVER_DOCS[word] },
        };
    }
...
connection.onDefinition((params) => {
    ...
    if (!sym)
        return null;
    return node_1.Location.create(sym.uri, node_1.Range.create(sym.line, sym.col, sym.line, sym.col + sym.name.length));
});
// ─── Boot ─────────────────────────────────────────────────────────────────────
